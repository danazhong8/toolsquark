...
function renderGuide(guide) {
  ...
  const toolPath = `<section class="tool-path-panel" aria-labelledby="tool-path-heading"><h2 id="tool-path-heading">Recommended Tool Path</h2><p>Use the guide to choose the right method, then open the matching tool when you are ready to calculate, reflect, or plan a next step.</p><div class="tool-path-grid">${tools}</div></section>`;
...
<div class="shell">
  ...
  <div class="layout">
    <article class="article">${toolPath}${article}<section class="faq"><h2>Frequently Asked Questions</h2>${faq}</section><section class="sources"><h2>Sources And Further Reading</h2><ul>${references}</ul></section><section class="related"><h2>Related Guides</h2><div class="related-grid">${related}</div></section><div class="disclaimer">These guides provide general education and help select a relevant tool. They do not diagnose a condition, prescribe treatment, or replace individualized professional guidance.</div></article>
    <aside class="sidebar"><section class="side-section"><h2>Tools Mentioned In This Guide</h2>${tools}</section><section class="side-section"><h2>Editorial Standard</h2><p style="font-size:13px;color:var(--muted)">Methods, limitations, references, and correction routes are disclosed under the <a href="../editorial-policy.html">Editorial & Methodology Policy</a>.</p></section></aside>
  </div>
</div>
${footer()}
<script>
