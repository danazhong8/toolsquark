...
function renderHome() {
  ...
  const directoryGroups = Object.entries(groups).map(([key, group]) => {
    const toolCount = group.sections.reduce((count, section) => count + section.tools.length, 0);
    const topics = group.sections.map((section) => `<div class="directory-topic"><h4>${esc(section.title)}</h4><ul>${section.tools.map((tool) => `<li><a href="tools/${tool[0]}.html">${esc(tool[1])}</a></li>`).join("")}</ul></div>`).join("");
    return `<section class="directory-group" aria-labelledby="directory-${key}"><div class="directory-intro"><h3 id="directory-${key}"><a href="${group.file}">${esc(group.categoryLabel)}</a></h3><p>${esc(categoryDescriptions[key])}</p><a href="${group.file}" class="directory-category-link">View all ${toolCount} tools &rarr;</a></div><div class="directory-topics">${topics}</div></section>`;
  }).join("");
